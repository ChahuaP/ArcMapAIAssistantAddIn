...
from server.precheck import resolve_layer_references  # noqa: E402
...
def _execute_operation(operation_id: str, arguments: Dict[str, Any]) -> dict:
    # Layer references must be resolved against the same serialized map state
    # used for dispatch; parallel tool calls must not invalidate each other.
    with _operation_lock:
        return _execute_operation_locked(operation_id, arguments)


def _execute_operation_locked(operation_id: str, arguments: Dict[str, Any]) -> dict:
    card = _catalog.get(operation_id)
    schema = card.get("parameters_schema", {})
    arguments = arguments or {}
    verdict = precheck(dict(card, parameters_schema=model_schema(schema)), arguments, None)
    if verdict["status"] == "proven":
        arguments = lower_arguments(arguments, schema)
        verdict = precheck(card, arguments, None)
    if verdict["status"] == "proven":
        try:
...
def _register_operation_tools() -> None:
    for operation_id in _catalog.operation_ids():
        ...
        run = codegen.build_operation_runner(operation_id, card, _execute_operation)
        run.__doc__ = description
        tool = Tool.from_function(run)
        tool.parameters = model_schema(card['parameters_schema'])
        mcp.add_tool(tool)

...
SCHEMA_VERSION = 1
SCHEMA_MARKER = "geopilot-journal-v1"
...
def _create_tables(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_context_snapshots_digest ON context_snapshots(digest);
...
class JournalStore:
    ...
    def session_messages(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        with self._connection() as conn:
            rows = conn.execute(
                "SELECT kind, stage, payload_json, recorded_at FROM run_events "
        ...
        return [
            {"kind": row[0], "stage": row[1], "payload": _json_loads(row[2]), "recorded_at": row[3]}
            for row in rows
        ]
        ...
        with self._connection() as conn:
            ...
            conn.execute(
                ...
                (run_id, request.session_id, request.request_id, request.caller.tenant_id,
                 RECEIVED, request.text, 1 if request.execute else 0, now, now),
            )
    ...
    def append_event(self, run_id: str, kind: str, stage: str,
        ...
        with self._connection() as conn:
            ...
            event_seq = self._append_event_locked(conn, run_id, kind, stage, payload, recorded_at)
            if outcome is not None:
                conn.execute(
                    """
    ...
    @staticmethod
    def _append_event_locked(conn: sqlite3.Connection, run_id: str, kind: str,
                             stage: str, payload: Dict[str, Any],
                             recorded_at: float) -> int:
        cursor = conn.execute(
            """
            INSERT INTO run_events(run_id, kind, stage, payload_json, recorded_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (run_id, kind, stage, _json_dumps(payload), recorded_at),
        )
    ...
    def delete_run(self, run_id: str) -> None:
        """Delete a terminal run and all its events/snapshots."""
        with self._connection() as conn:
            conn.execute("DELETE FROM run_events WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM context_snapshots WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM capability_snapshots WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM intent_specs WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM verified_plans WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM runtime_leases WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM authorization_grants WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM execution_receipts WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM artifacts WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM acceptance_reports WHERE run_id = ?", (run_id,))
            conn.execute("DELETE FROM publication_receipts WHERE run_id = ?", (run_id,))
    ...
    def run_events(self, run_id: str) -> List[Dict[str, Any]]:
        ...
        return [
            {"event_seq": row[0], "kind": row[1], "stage": row[2],
             "payload": _json_loads(row[3]), "recorded_at": row[4]}
            for row in rows
        ]

    def events_after(self, last_seq: int, limit: int = 200) -> List[Dict[str, Any]]:
        ...
        with ``Last-Event-ID`` never loses events.
        """
        ...
        return [
            {"event_seq": row[0], "run_id": row[1], "kind": row[2],
             "stage": row[3], "payload": _json_loads(row[4]), "recorded_at": row[5]}
            for row in rows
        ]

    # -- sealed snapshots (§4) ---------------------------------------------

    def store_context_snapshot(self, run_id: str, snapshot: contracts.ContextSnapshot) -> None:
        document = _seal_snapshot(snapshot)
        with self._connection() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO context_snapshots(run_id, digest, lease_id,
    ...
    def get_context_snapshot(self, run_id: str) -> Optional[contracts.ContextSnapshot]:
        with self._connection() as conn:
            row = conn.execute(
                "SELECT document_json FROM context_snapshots WHERE run_id = ? "
    ...
    def store_artifact(self, artifact_id: str, run_id: str, kind: str,
                       path: Optional[str], artifact_digest: str,
                       document: Dict[str, Any], staged: bool = True) -> None:
        with self._connection() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO artifacts(artifact_id, run_id, kind, path, digest,
                                                staged, published, document_json, created_at)
                VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?)
                """,
                (artifact_id, run_id, kind, path, artifact_digest,
                 1 if staged else 0, _json_dumps(document), _now()),
            )
    ...
    def store_publication_receipt(self, publication_id: str, run_id: str,
                                  grant_id: str, document: Dict[str, Any]) -> None:
        with self._connection() as conn:
            conn.execute(
                """
                INSERT OR REPLACE INTO publication_receipts(run_id, publication_id, grant_id,
                                                          document_json, published_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (run_id, publication_id, grant_id, _json_dumps(document), _now()),
            )
    ...
    def list_staged_artifacts(self, run_id: str) -> List[Dict[str, Any]]:
        ...
        with self._connection() as conn:
            rows = conn.execute(
                "SELECT artifact_id, kind, path, digest, document_json "
                "FROM artifacts WHERE run_id = ? AND staged = 1 AND published = 0 "
                "ORDER BY created_at",
                (run_id,),
            ).fetchall()
        return [
            {"artifact_id": row[0], "kind": row[1], "path": row[2],
             "digest": row[3], "document": _json_loads(row[4])}
            for row in rows
        ]
    ...
    def reserve_model_call(self, call_key: str, run_id: str, provider: str,
        ...
        Only a ``reserved`` record (another caller is in-flight) blocks. A
        succeeded record never reaches here (cache lookup reuses it). A
        failed/quota_stopped/uncertain record is overwritten so the caller can
        retry without leaving a dead ledger row.
        """
        with self._connection() as conn:
            conn.execute("BEGIN IMMEDIATE")
            ...
            if existing is not None and existing[0] == "reserved":
                return False
            conn.execute(
                """
    ...
    def get_model_call(self, call_key: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "response": _json_loads(row[6]) if row[6] else None,
            "ledger": _json_loads(row[7]), "reserved_at": row[8], "committed_at": row[9],
        }
    ...
    def commit_model_call(self, call_key: str, status: str,
        ...
        with self._connection() as conn:
            ...
            conn.execute(
                ...
                """,
                (status, _json_dumps(response) if response is not None else None,
                 _json_dumps(ledger), _now(), call_key),
            )

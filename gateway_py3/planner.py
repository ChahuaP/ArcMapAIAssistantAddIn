...
- Custom tool executor_code receives arguments["output_path"] as a Unicode Windows path. Use it directly; do not call str(), unicode(), encode(), decode(), or sys.getfilesystemencoding() to repair Chinese paths.
...
- Use output_policy.type deliberately: feature_class for ArcGIS vector outputs with gdb/shp formats, file for ordinary files such as .obj/.json/.csv with a declared extension, and raster for .tif raster outputs. File outputs may call open(arguments["output_path"], "w" or "wb") and must not open any other path; GeoPilot's injected open handles Chinese paths.

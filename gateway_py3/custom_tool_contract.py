...
- If an enabled custom.* operation already matches the user goal, generate a workflow using that operation; do not create or revise a custom tool just to run it.
...
- The runtime injects arguments["output_path"] as a Python 2.7 Unicode Windows path. Executor code must use it directly; do not call str(), unicode(), encode(), decode(), or sys.getfilesystemencoding() to repair paths.
...
- For file outputs, operation_spec.output_policy must include extension such as ".obj"; executor_code may call open(arguments["output_path"], "w") or open(arguments["output_path"], "wb") and must not open any other path. The injected open handles Chinese output paths.
...
- It is allowed to split arguments["output_path"] with os.path.dirname/basename when ArcPy requires workspace and dataset name separately; GeoPilot injects a Unicode-safe os.path wrapper.

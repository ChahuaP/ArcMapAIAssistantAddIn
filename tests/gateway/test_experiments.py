...
CONTEXT = {"layers": []}
WORKFLOW = {"action": "execute", "summary": "refresh", "steps": [{"id": "s1", "operation": "view.refresh_view", "arguments": {}, "reason": "refresh"}]}
SEMANTICS = {"goal": "refresh", "inputs": [], "constraints": [], "success_criteria": []}
...
class ExperimentTests(unittest.TestCase):
    def runner(self, replies):
        ...
        return ExperimentRunner(OperationCatalog(), RunStore(Path(self.temp.name) / "runs.sqlite"), FakeModel(replies))
    ...
    def test_g0_hides_context_but_validator_receives_it(self):
        ...
        request = json.loads(runner.client.calls[0][1]["content"])
    ...
    def test_g1_has_one_context_role(self):
        ...
        self.assertIn('"context"', runner.client.calls[0][1]["content"])
    ...
    def test_g3_isolates_roles_and_requires_audit_pass(self):
        ...
        runner = self.runner([{"task_semantics": SEMANTICS}, {"workflow_draft": WORKFLOW}, {"audit_result": revise}, {"workflow_draft": WORKFLOW}, {"audit_result": passed}])
        row = plan_bound(runner, "refresh", CONTEXT, "multi_agent")
        trace = row["agent_trace"][0]["run"]
        self.assertEqual(trace["counts"]["revisions"], 1)
        self.assertEqual(trace["audits"][-1]["decision"], "pass")
        self.assertEqual([call[0]["content"].split("GeoPilot ")[1].split(" role")[0] for call in runner.client.calls], ["semantic", "planner", "auditor", "planner", "auditor"])

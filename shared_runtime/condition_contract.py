...
def canonical_operator(condition, strict=True, error_cls=ValueError, missing_message=u"属性条件缺少 op。"):
    ...
    if op is None:
        ...
        return ""
    if not isinstance(op, _STRING_TYPES):
        if strict:
            raise error_cls("condition op must be a string.")
        return ""
    return op
...
def validate_condition_tree(condition, error_cls=ValueError):
    ...
    if op == "not":
        ...
        return condition
    if op not in LEAF_CONDITION_OPERATORS or not isinstance(condition.get("field"), _STRING_TYPES) or not condition["field"].strip():
    ...
    if op in VALUE_CONDITION_OPERATORS:
        ...
        if has_value_field and (not isinstance(condition["value_field"], _STRING_TYPES) or not condition["value_field"].strip()):

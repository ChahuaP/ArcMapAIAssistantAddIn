...
2. Run `scripts/geopilot_cli.py arcmap-list` to see all open ArcMap instances. If more than one is live, ask the user which target to use, then run `scripts/geopilot_cli.py arcmap-select --hwnd <hwnd>`.
   - The Gateway starts `ArcMapBridge.exe` on first ArcMap access; ArcMap launch alone does not keep the Bridge running.
...
3. Run `scripts/geopilot_cli.py arcmap-sync` to make the selected ArcMap synchronize the current context.
4. Run `scripts/geopilot_cli.py capabilities` and choose only registered operations.
5. Draft a workflow JSON locally. For non-trivial planning, read `references/planning-contract.md` and `references/workflow-format.md`.
6. Validate with `scripts/geopilot_cli.py validate --workflow workflow.json`.
7. If validation fails, fix the workflow and validate again.
8. If the user confirmed this run, execute with `scripts/geopilot_cli.py arcmap-execute-workflow --confirmed --command "<user request>" --workflow workflow.json`.
9. If the user enabled full auto, first run `scripts/geopilot_cli.py arcmap-permission --auto-execute`, then execute without asking again.
...
- Do not manually start `ArcMapBridge.exe`; access ArcMap through the Gateway so it can own Bridge startup, target discovery, and shutdown.
